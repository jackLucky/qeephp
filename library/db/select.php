...
class QDB_Select
{
    ...
    protected function _renderFrom()
    {
        ...
        foreach ($this->_parts[self::FROM] as $alias => $table)
        {
            ...
            {
                $tmp .= $this->_conn->qid("{$table['schema']}.{$table['table_name']}", $alias, 'AS');
            }
    ...
    protected function _renderHaving()
    {
        ...
        {
        	$having = $this->_parts[self::HAVING]->formatToString($this->_conn, $this->_getCurrentTableName(), null, array(
        			$this, '_parseTableName'
        	));
        	if (! empty($having))
        	{
    ...
    protected function _getCurrentTableName()
    {
        ...
        {
            list ($alias, ) = each($this->_current_table);
