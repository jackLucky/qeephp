...
class Model_Behavior_AclUser extends QDB_ActiveRecord_Behavior_Abstract
{
    ...
    (
        ...
        //2011年03月15日 吴魁修改,第二认证字段
        'second_username_prop'      => 'email',
        'password_prop'             => 'password',

        'roles_enabled'             => true,
        'roles_prop'                => 'roles',
        'roles_name_prop'           => 'rolename',
        ...
        //2011年02月20日 吴魁修改,自动保存session_id
        'update_login_session_id_prop' => null,
     ...
     * @param string $username_prop 认证属性
     ...
     */
    function validateLogin($username, $password, $username_prop = 'FIRST', $update_login = true)
    {
    	if($username_prop == "FIRST")
    	{
    		$pn = $this->_settings['username_prop'];
    	}
    	elseif ($username_prop == "SECOND")
    	{
    		$pn = $this->_settings['second_username_prop'];
    	}
        ...
        if($this->_settings['update_login_session_id_prop'] && 
           $member[$this->_settings['update_login_session_id_prop']])
        {
        	$this->_settings['last_login_session'] = $member[$this->_settings['update_login_session_id_prop']];
        }
    ...
    function updateLoginDyn(QDB_ActiveRecord_Abstract $member, array $data = null)
    {
        ...
        //2011年02月20日 吴魁修改,自动保存session_id
        $pn = $this->_settings['update_login_session_id_prop'];
        if ($pn)
        {
        	$session_id = isset($data['login_at']) ?
        	              $data['login_at']
        	              : session_id();
        	$member->changePropForce($pn, $session_id);
        }
