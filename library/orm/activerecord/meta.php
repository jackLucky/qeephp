...
class QDB_ActiveRecord_Meta implements QDB_ActiveRecord_Callbacks
{
    ...
    function relatedObjects(QDB_ActiveRecord_Abstract $obj, $prop_name)
    {
        ...
        case QDB::BELONGS_TO:
             ...
             * @var QDB_Select
             */
            $select = $target_meta->find(array($assoc->target_key => $source_key_value))->setColumns($assoc->on_find_keys);
            break;
        ...
        if( !empty($assoc->on_find_group) )
        {
        	$select->group($assoc->on_find_group);
        }
