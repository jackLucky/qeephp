...
class QDB_ActiveRecord_Association_ManyToMany extends QDB_ActiveRecord_Association_Abstract
{
    ...
    protected function _updateRelationByMeta(QDB_ActiveRecord_Abstract $source, $targets, $recursion)
    {
        ...
        foreach ($targets as $target)
        {
            ...
            if (!$exists->search($this->mid_target_key, $v))//将$this->target_key 改成$this->mid_target_key，应该是作者写错了
            {
                $added[] = $v;
            }
        }
        ...
        {
        	$v = $exist->{$this->mid_target_key};//将$this->target_key 改成$this->mid_target_key，应该是作者写错了
        	if (!isset($targets[$v]))
        	{
        		$removed[] = $v;
        	}
        ...
        if (!empty($removed))
        {
            $this->mid_meta->destroyWhere("{$this->mid_source_key} = ? AND {$this->mid_target_key} IN (?)", $source_key_value, $removed);
        }
